...
must_haves:
  truths:
    - "App displays text in user's configured language"
    - "App loads with English locale by default when no setting exists"
    - "Locale can be changed via LocaleProvider and UI updates immediately"
...
3. Update `mobile/lib/app.dart`:
  ...
  <verify>
    - `cd mobile && flutter build apk --debug` completes without errors
    - `grep "LocaleProvider" mobile/lib/app.dart` shows provider is wired
    - `grep "localizationsDelegates" mobile/lib/app.dart` shows delegates are configured
    - `grep "AppLocalizations.delegate" mobile/lib/app.dart` shows AppLocalizations is used
    - App launches with `flutter run` and displays in English by default
  </verify>
